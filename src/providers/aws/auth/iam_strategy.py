...
class IAMAuthStrategy(AuthPort):
    ...
    async def authenticate(self, context: AuthContext) -> AuthResult:
        ...
        if not self.enabled:
            return AuthResult(
                ...
                error_message="IAM authentication is disabled"
            )
        ...
        try:
            ...
            return AuthResult(
                status=AuthStatus.FAILED,
                error_message=f"AWS IAM error: {error_code}"
            )
        except Exception as e:
            self._logger.error(f"IAM authentication error: {e}")
            return AuthResult(
                status=AuthStatus.FAILED,
                error_message="IAM authentication failed"
            )
    ...
    async def validate_token(self, token: str) -> AuthResult:
        ...
        return await self.authenticate(AuthContext(
            method="GET",
            path="/validate",
    ...
    async def _determine_roles(self, 
