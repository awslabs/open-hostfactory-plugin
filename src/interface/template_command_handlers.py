...
async def handle_list_templates(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    try:
        ...
        if scheduler_strategy:
            formatted_response = scheduler_strategy.convert_domain_to_hostfactory_output("getAvailableTemplates", templates)
            templates_data = formatted_response.get("templates", [])
        else:
            templates_data = [template.model_dump() if hasattr(template, 'model_dump') else template for template in templates]
...
async def handle_create_template(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    try:
        ...
        if not command_bus:
            return {
                "success": False,
                "error": "CommandBus not available"
            }
        ...
        if not provider_api:
            return {
                "success": False,
                "error": "Provider API is required"
            }
...
async def handle_refresh_templates(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Failed to refresh templates: {str(e)}"
        }
