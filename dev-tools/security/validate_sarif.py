...
class SarifValidator:
    ...
    def validate_sarif_structure(self, sarif_data: Dict) -> bool:
        ...
        if sarif_data.get("version") != "2.1.0":
            self.warnings.append(f"SARIF version {sarif_data.get('version')} may not be fully supported. Recommended: 2.1.0")
    ...
    def validate_result(self, result: Dict, run_index: int, result_index: int) -> bool:
        ...
        for field in required_fields:
            if field not in result:
                self.errors.append(f"Run {run_index}, Result {result_index}: Missing required field: {field}")
                return False
...
def main():
    ...
    if all_valid:
        ...
        print("Some SARIF files have issues. Please fix them before uploading to GitHub Security tab.")
        sys.exit(1)
