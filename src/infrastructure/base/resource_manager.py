...
from src.domain.base.resource_manager import (
    ResourceManagerPort, ResourceSpecification, ResourceAllocation, 
...
class BaseResourceManager(ResourceManagerPort, ABC):
    ...
    async def provision_resources(self, specification: ResourceSpecification) -> ResourceAllocation:
        ...
        if self.logger:
            self.logger.info(f"Starting resource provisioning: {specification.name} ({specification.resource_type.value})")
    ...
    async def deprovision_resources(self, allocation: ResourceAllocation) -> None:
        ...
        try:
            ...
            duration = time.time() - start_time
            if self.logger:
                self.logger.error(f"Resource deprovisioning failed: {allocation.resource_id} in {duration:.3f}s - {str(e)}")
