...
from src.infrastructure.resilience.exceptions import InvalidRetryStrategyError, MaxRetriesExceededError

T = TypeVar('T')
logger = get_logger(__name__)


def retry(strategy: str = "exponential",
          max_attempts: int = 3,
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        if strategy == "exponential":
            ...
            raise InvalidRetryStrategyError(f"Unsupported retry strategy: {strategy}. Supported: exponential, circuit_breaker")
                ...
                try:
                    ...
                    if not retry_strategy.should_retry(attempt, e):
                        if attempt >= max_attempts:
                            logger.error(f"Max retry attempts ({max_attempts}) exceeded for {func.__name__}: {e}")
