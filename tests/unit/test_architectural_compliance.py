...
class TestDDDCompliance:
    ...
    def test_domain_has_no_infrastructure_dependencies(self):
        ...
        assert not infrastructure_imports, f"Domain layer has infrastructure dependencies: {infrastructure_imports}"
                ...
                try:
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.Import):
                            ...
                            if node.module and ('providers' in node.module or 'aws' in node.module.lower()):
                            ...
                            if isinstance(node, ast.ImportFrom):
                                if node.module:
                                    for other_context in contexts:
                                        if other_context != context and f"domain.{other_context}" in node.module:
                                            violations.append(f"{py_file}: imports from {other_context} context")
                    except SyntaxError:
                        continue
...
class TestSOLIDCompliance:
    ...
    def test_single_responsibility_principle(self):
        ...
        methods = [method for method in dir(ApplicationService) 
        ...
        orchestration_methods = [
            'get_available_templates', 'get_template_by_id', 'request_machines',
            'get_request_status', 'request_return_machines', 'get_return_requests',
            'get_machine_status', 'get_machines_by_request', 'validate_template',
            'get_provider_health', 'get_provider_info'
        ]
    ...
    def test_open_closed_principle(self):
        ...
        assert hasattr(ProviderPort, '__abstractmethods__') or \
               hasattr(ProviderPort, '_abc_registry'), \
    ...
    def test_interface_segregation_principle(self):
        ...
        methods = [method for method in dir(ProviderPort) 
    ...
    def test_dependency_inversion_principle(self):
        ...
        for param_name, param in sig.parameters.items():
            ...
            if param.annotation != inspect.Parameter.empty:
                ...
                assert 'Port' in annotation_str or 'Interface' in annotation_str or \
                       'Service' in annotation_str or 'Bus' in annotation_str, \
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.ImportFrom):
                            if node.module and not node.module.startswith('src.domain'):
                                ...
                                if not any(node.module.startswith(allowed) for allowed in 
                                         ['typing', 'datetime', 'uuid', 'enum', 'abc', 'dataclasses', 'pydantic']):
                                    external_deps.append(f"{py_file}: {node.module}")
                except SyntaxError:
                    continue
        
        # Domain should only depend on standard library and domain-specific libraries
        assert len(external_deps) < 5, f"Domain layer has too many external dependencies: {external_deps}"
...
class TestDesignPatternCompliance:
    ...
    def test_aggregate_pattern_compliance(self):
        ...
        assert issubclass(Request, AggregateRoot), \
...
class TestCodeQualityCompliance:
    ...
    def test_consistent_naming_conventions(self):
        ...
        assert not aws_handler_usage, f"Found aws_handler usage (should be provider_api): {aws_handler_usage}"
