    ...
    try:
        ...
        if _is_optional_type(annotation):
            ...
            if _is_primitive_type(inner_type):
                logger.debug(f"Skipping primitive type resolution for {param_name}: {inner_type.__name__} in {class_name}")
                return param.default if param.default != inspect.Parameter.empty else None
            
            try:
                return container.get(inner_type)
            except Exception as e:
                logger.debug(f"Could not resolve optional dependency {param_name}: {inner_type.__name__} for {class_name}: {e}")
                return param.default if param.default != inspect.Parameter.empty else None
