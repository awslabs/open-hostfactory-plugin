...
from src.infrastructure.registry.enhanced_base_registry import (
    EnhancedBaseRegistry, RegistryMode, BaseRegistration
)
...
class TestEnhancedBaseRegistry:
    ...
    def test_single_choice_mode_prevents_instance_registration(self):
        ...
        with pytest.raises(ValueError, match="Instance registration only supported in MULTI_CHOICE mode"):
            registry.register_instance("json", "json-primary", self.mock_strategy_factory, self.mock_config_factory)
    ...
    def test_single_choice_mode_prevents_instance_creation(self):
        ...
        with pytest.raises(ValueError, match="Instance-based creation only supported in MULTI_CHOICE mode"):
    ...
    def test_multi_choice_mode_instance_registration(self):
        ...
        # Register instances
        registry.register_instance("aws", "aws-primary", self.mock_strategy_factory, self.mock_config_factory)
        registry.register_instance("aws", "aws-secondary", self.mock_strategy_factory, self.mock_config_factory)
        ...
        primary_strategy = registry.create_strategy_by_instance("aws-primary", {"config": "primary"})
        secondary_strategy = registry.create_strategy_by_instance("aws-secondary", {"config": "secondary"})
