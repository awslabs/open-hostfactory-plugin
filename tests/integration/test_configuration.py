...
def test_aws_provider_config_class():
    ...
    try:
        ...
        if lt_config.naming_strategy != "request_based":
            print("   FAIL: LaunchTemplateConfiguration default naming_strategy should be 'request_based'")
            return False
        
        if lt_config.max_versions_per_template != 10:
            print("   FAIL: LaunchTemplateConfiguration default max_versions_per_template should be 10")
            return False
        ...
        if not isinstance(aws_config.launch_template, LaunchTemplateConfiguration):
            print("   FAIL: AWSProviderConfig.launch_template is not LaunchTemplateConfiguration instance")
            return False
...
def test_configuration_loading():
    ...
    try:
        ...
        print(f"   📋 Launch template create_per_request: {aws_config.launch_template.create_per_request}")
