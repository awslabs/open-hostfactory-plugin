...
class BaseProviderHandler(Generic[TRequest, TResponse], ProviderHandler[TRequest, TResponse], ABC):
    ...
    async def handle(self, request: TRequest, context: Optional[ProviderContext] = None) -> TResponse:
        ...
        try:
            ...
            if self.logger:
                self.logger.info(f"{self.provider_type.upper()} provider request processed successfully: {request_type} ({duration:.3f}s)")
            ...
            if self.logger:
                self.logger.error(f"{self.provider_type.upper()} provider request processing failed: {request_type} - {str(e)}")
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries and self.should_retry(e):
                    ...
                    if self.logger:
                        self.logger.warning(f"AWS request failed (attempt {attempt + 1}/{self.max_retries + 1}), retrying in {delay}s: {str(e)}")
