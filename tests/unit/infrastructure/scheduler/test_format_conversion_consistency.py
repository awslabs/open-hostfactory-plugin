...
class TestFormatConversionConsistency:
    ...
    def test_format_return_request_response_consistency(self):
        ...
        # Act
        default_result = self.default_strategy.format_return_request_response(self.sample_return_request)
        symphony_result = self.symphony_strategy.format_return_request_response(self.sample_return_request)
        ...
        assert len(symphony_request["machine_ids"]) == len(self.sample_return_request["machine_ids"])


class TestFormatConversionInHandlers:
    """Test that format conversion is used consistently in handlers."""

    @patch('src.api.handlers.get_available_templates_handler.GetAvailableTemplatesRESTHandler._handle')
    def test_format_conversion_in_api_handler(self, mock_handle):
        ...
        formatted_templates = {"templates": [{"id": "template1", "formatted": True}, {"id": "template2", "formatted": True}]}
