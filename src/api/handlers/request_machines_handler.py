...
class RequestMachinesRESTHandler(BaseAPIHandler[RequestMachinesModel, RequestMachinesResponse]):
    ...
    async def execute_api_request(self, request: RequestMachinesModel, context) -> RequestMachinesResponse:
        ...
        """
        if self.logger:
            self.logger.info(f"Processing request machines - Template: {request.template_id}, Count: {request.max_number} - Correlation ID: {context.correlation_id}")
        ...
        try:
            ...
            if self.logger:
                self.logger.info(f"Successfully submitted machine request: {request_id} - Correlation ID: {context.correlation_id}")
