...
class ProviderSelectionService:
    ...
    def _get_enabled_instances_by_type(self, provider_type: str) -> List[ProviderInstanceConfig]:
        ...
        return [
            provider for provider in self._provider_config.providers
    ...
    def _weighted_round_robin_selection(self, instances: List[ProviderInstanceConfig]) -> ProviderInstanceConfig:
        ...
        highest_priority_instances = [
            instance for instance in sorted_instances 
        ...
        if len(highest_priority_instances) == 1:
            ...
            self._logger.debug(f"Selected provider {selected.name} (priority {selected.priority}, weight {selected.weight})")
            return selected
        ...
        self._logger.debug(f"Selected provider {selected.name} (priority {selected.priority}, weight {selected.weight}) from {len(highest_priority_instances)} candidates")
        return selected
