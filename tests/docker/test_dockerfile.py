...
class TestDockerfile:
    ...
    def test_dockerfile_structure(self, dockerfile_path):
        ...
        assert "RUN groupadd -r ohfp && useradd -r -g ohfp" in content, "Should create non-root user"
    ...
    def test_docker_build_success(self, project_root):
        ...
        try:
            result = subprocess.run([
                "docker", "build", 
                "-t", "ohfp-api:test-build",
                "--build-arg", "BUILD_DATE=2025-01-07T00:00:00Z",
    ...
    def test_container_startup(self, project_root):
        ...
        try:
            ...
            subprocess.run([
                "docker", "build", "-t", "ohfp-api:test-startup", 
                "--quiet", str(project_root)
    ...
    def test_container_health_check(self, project_root):
        ...
        try:
            # Build image
            subprocess.run([
                "docker", "build", "-t", "ohfp-api:test-health", 
                "--quiet", str(project_root)
            ], check=True, capture_output=True)
            
            # Start container
            container_result = subprocess.run([
                "docker", "run", "-d", "--name", "ohfp-health-test",
                "-p", "8002:8000",
                "-e", "HF_SERVER_ENABLED=true",
                "-e", "HF_AUTH_ENABLED=false",
                "ohfp-api:test-health", "serve"
