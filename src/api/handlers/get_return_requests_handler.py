...
class GetReturnRequestsRESTHandler(BaseAPIHandler[Dict[str, Any], ReturnRequestResponse]):
        ...
        try:
            ...
            for req in return_requests:
                request_data = {
                    ...
                    "createdAt": req.created_at.isoformat() if hasattr(req.created_at, 'isoformat') else req.created_at
                }
                
                if long:
                    request_data.update({
                        "machines": [m.to_dict() if hasattr(m, 'to_dict') else m for m in req.machines] if hasattr(req, 'machines') else [],
                        "metadata": req.metadata if hasattr(req, 'metadata') else {},
                        "events": [e.to_dict() if hasattr(e, 'to_dict') else e for e in req.events] if hasattr(req, 'events') else []
                    })
    ...
    async def post_process_response(self, response: ReturnRequestResponse, context: RequestContext) -> ReturnRequestResponse:
        """
