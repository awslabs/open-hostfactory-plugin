...
from src.domain.base.events import RequestCreatedEvent, RequestStatusChangedEvent, RequestCompletedEvent
...
class Request(AggregateRoot):
    ...
    def is_complete(self) -> bool:
        """Check if request is complete."""
        return self.status in [RequestStatus.COMPLETED, RequestStatus.FAILED, RequestStatus.CANCELLED, RequestStatus.PARTIAL]
    ...
    def create_return_request(cls,
        ...
        request = cls(
            ...
            provider_type=machine_refs[0].get('provider_type', 'unknown') if machine_refs else 'unknown',  # Extract from machine refs
            status=RequestStatus.PENDING,
