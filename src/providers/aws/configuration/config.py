...
class HandlersConfig(BaseModel):
    ...
    def sync_legacy_fields(self) -> 'HandlersConfig':
        ...
        if (self.ec2_fleet != self.capabilities.ec2_fleet or
            self.spot_fleet != self.capabilities.spot_fleet or
            self.auto_scaling_group != self.capabilities.auto_scaling_group or
            self.run_instances != self.capabilities.run_instances):
...
class AWSProviderConfig(BaseProviderConfig):
    ...
    service_role_spot_fleet: str = Field(
        ...
        description="Service role for Spot Fleet"
    )
