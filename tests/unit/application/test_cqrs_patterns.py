...
class TestCommandQuerySeparation:
    ...
    def test_commands_do_not_return_business_data(self):
        ...
        command_methods = [method for method in dir(command) 
    ...
    def test_queries_do_not_modify_state(self):
        ...
        query_methods = [method for method in dir(query) 
                        if not method.startswith('_') and callable(getattr(query, method))]
        
        # Queries should not have "set", "update", "create", "delete" methods
        modifying_methods = [method for method in query_methods 
                           if any(verb in method.lower() for verb in ['set', 'update', 'create', 'delete', 'modify'])]
        assert len(modifying_methods) == 0, f"Queries should not have modifying methods: {modifying_methods}"
    ...
    def test_command_handlers_modify_state(self):
        ...
        handler = CreateRequestHandler(
            request_repository=mock_repository,
...
class TestCQRSIntegration:
    ...
    def test_cqrs_supports_read_models(self):
        ...
        handler = GetRequestStatusHandler(
            request_repository=mock_read_repository
        )
