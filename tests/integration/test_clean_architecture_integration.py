...
class TestCleanArchitectureIntegration:
    ...
    def mock_config_manager(self):
        ...
        aws_provider.template_defaults = {
            'instance_type': 't3.small'  # Override
        }
        aws_provider.extensions = {
            'volume_type': 'gp2'  # Override
        }
    ...
    def test_clean_template_schema_validation(self):
        ...
        aws_specific_fields = [
            'ami_resolution', 'default_fleet_role', 'default_volume_type',
            'default_allocation_strategy', 'subnet_ids', 'security_group_ids'
        ]
        for field in aws_specific_fields:
            assert field not in config_dict, f"AWS-specific field {field} should not be in clean template config"
    ...
    def test_effective_template_resolution(self, template_defaults_service):
        ...
        template_dict = {
            ...
            'max_instances': 5  # Template override
        }
    ...
    def test_provider_strategy_schema_extensions(self):
        ...
        from src.config.schemas.provider_strategy_schema import ProviderDefaults, ProviderInstanceConfig
