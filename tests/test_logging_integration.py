...
class TestLoggingIntegration:
    ...
    def test_templates_list_logging_output(self, complete_test_environment):
        ...
        health_check_errors = [line for line in log_lines if "Error checking health of strategy" in line]
        ...
        ssm_resolution_lines = [line for line in log_lines if "Resolved SSM parameter" in line and "to AMI" in line]
        
        # Should have exactly one SSM resolution log (no duplicates)
        assert len(ssm_resolution_lines) <= 1, f"Found duplicate SSM resolution: {ssm_resolution_lines}"
        ...
        batch_resolution_lines = [line for line in log_lines if "Batch resolved" in line and "unique SSM parameters" in line]
    ...
    def test_logging_performance(self, complete_test_environment):
        ...
        assert len(log_lines) < 150, f"Too many log messages ({len(log_lines)}), possible logging spam"
        
        # Should have some log messages (not silent)
        assert len(log_lines) > 10, f"Too few log messages ({len(log_lines)}), possible logging issues"
