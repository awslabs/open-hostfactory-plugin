...
class GetRequestHandler(BaseQueryHandler[GetRequestQuery, RequestDTO]):
    ...
    async def execute_query(self, query: GetRequestQuery) -> RequestDTO:
        ...
        try:
            ...
            if not machines and request.resource_ids:
                self.logger.info(f"DEBUG: No machines in storage but have resource IDs {request.resource_ids}, checking provider")
    ...
    async def _check_provider_and_create_machines(self, request) -> List:
        ...
        try:
            ...
            self.logger.info(f"Provider strategy result: success={result.success}, data_keys={list(result.data.keys()) if result.data else 'None'}")
        ...
        try:
            ...
            for machine in machines:
                ...
                if domain_machine:
                    ...
                    # Check if we need to update the machine (status or network info changed)
                    needs_update = (
                        machine.status != new_status or
                        machine.private_ip != domain_machine.get('private_ip') or
                        machine.public_ip != domain_machine.get('public_ip')
                    )
...
class GetTemplateHandler(BaseQueryHandler[GetTemplateQuery, Template]):
    ...
    async def execute_query(self, query: GetTemplateQuery) -> Template:
        ...
        try:
            ...
            # Convert TemplateDTO to Template domain object (same logic as ListTemplatesHandler)
