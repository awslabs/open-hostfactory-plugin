...
class BaseAPIHandler(BaseHandler, Generic[T, R]):
    ...
    def apply_middleware(self, func: Callable[[T], R], service_name: Optional[str] = None) -> Callable[[T], Dict[str, Any]]:
        """
    ...
    def with_retry_middleware(self, service_name: str = "api",
                             max_attempts: int = 3,
                             base_delay: float = 1.0) -> Callable[[Callable[[T], R]], Callable[[T], R]]:
        """
        ...
        def decorator(func: Callable[[T], R]) -> Callable[[T], R]:
            @retry(strategy="exponential", max_attempts=max_attempts, 
                   base_delay=base_delay, service=service_name)
            def wrapper(request: T) -> R:
                return func(request)
            return wrapper
        return decorator
