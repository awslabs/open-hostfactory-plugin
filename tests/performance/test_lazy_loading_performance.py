...
class TestLazyLoadingPerformance:
    ...
    def test_first_command_performance(self):
        ...
        assert first_access_time < 1000, f"First query bus access took {first_access_time:.1f}ms, expected <1000ms"
    ...
    def test_component_registration_performance(self):
        ...
        assert registration_time < 200, f"Service registration took {registration_time:.1f}ms, expected <200ms"
    ...
    def test_handler_discovery_performance(self):
        ...
        assert discovery_time < 500, f"Handler discovery took {discovery_time:.1f}ms, expected <500ms"
        assert results['total_handlers'] >= 50, f"Expected ≥50 handlers, got {results['total_handlers']}"
        print(f"✅ Handler discovery: {discovery_time:.1f}ms for {results['total_handlers']} handlers")
    ...
    def test_storage_registration_performance(self):
        ...
        assert registration_time < 100, f"Minimal storage registration took {registration_time:.1f}ms, expected <100ms"
    ...
    def test_scheduler_registration_performance(self):
        ...
        assert registration_time < 50, f"Active scheduler registration took {registration_time:.1f}ms, expected <50ms"
    ...
    def test_concurrent_access_performance(self):
        ...
        assert concurrent_time < 3000, f"Concurrent access took {concurrent_time:.1f}ms, expected <3000ms"
