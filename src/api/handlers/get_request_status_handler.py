...
class GetRequestStatusRESTHandler(BaseAPIHandler[Dict[str, Any], RequestStatusResponse]):
                        ...
                        if self.logger:
                            self.logger.info(
                                ...
                                extra={
                                    ...
                                    'correlation_id': correlation_id,
                                    'status': request_data.status.value if hasattr(request_data, 'status') and hasattr(request_data.status, 'value') and not isinstance(request_data.status, str) else request_data.status if hasattr(request_data, 'status') else request_data
                                }
                            )
                        
                        requests.append(request_data.to_dict() if hasattr(request_data, 'to_dict') else 
                                      {"requestId": request_id, "status": request_data})
    ...
    async def post_process_response(self, response: RequestStatusResponse, context: RequestContext) -> RequestStatusResponse:
        """
