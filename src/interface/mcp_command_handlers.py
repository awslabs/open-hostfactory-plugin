    ...
    async with OpenHFPluginMCPTools() as tools:
        ...
        if hasattr(args, 'type') and args.type:
            ...
            for tool in tool_list:
                ...
                if (tool_def and tool_def.method_info and 
                    tool_def.method_info.handler_type == args.type):
...
def _format_tools_table(tools: list) -> Dict[str, Any]:
    ...
    for tool in tools:
        ...
        rows.append([
            tool["name"],
            tool.get("description", "No description")[:60] + ("..." if len(tool.get("description", "")) > 60 else ""),
            handler_type
        ])
