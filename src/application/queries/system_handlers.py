...
class GetProviderConfigHandler(BaseQueryHandler[GetProviderConfigQuery, ProviderConfigDTO]):
    ...
    def __init__(self, logger: LoggingPort, container: ContainerPort, error_handler: ErrorHandlingPort):
        """
        ...
        try:
            ...
            if hasattr(config_manager, 'get_provider_config'):
                ...
                return ProviderConfigDTO(
                    provider_mode=provider_config.get_mode().value if hasattr(provider_config, 'get_mode') else "legacy",
                    active_providers=[p.name for p in provider_config.get_active_providers()] if hasattr(provider_config, 'get_active_providers') else [],
                    provider_count=len(provider_config.get_active_providers()) if hasattr(provider_config, 'get_active_providers') else 0,
...
class ValidateProviderConfigHandler(BaseQueryHandler[ValidateProviderConfigQuery, ValidationResultDTO]):
    ...
    async def execute_query(self, query: ValidateProviderConfigQuery) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                provider_config = config_manager.get_provider_config() if hasattr(config_manager, 'get_provider_config') else None
