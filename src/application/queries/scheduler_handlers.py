...
class GetSchedulerConfigurationHandler(BaseQueryHandler[GetSchedulerConfigurationQuery, SchedulerConfigurationResponse]):
    ...
    async def execute_query(self, query: ValidateSchedulerConfigurationQuery) -> ValidationResultDTO:
        ...
        try:
            ...
            if scheduler_name not in registered_types:
                errors.append(f"Scheduler '{scheduler_name}' is not registered. Available: {', '.join(registered_types)}")
