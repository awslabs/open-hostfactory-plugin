"""Provider handler base classes for the domain layer.

These types define the provider-handler contract and its shared template-method
implementation.  They live in the domain layer so that provider implementations
(``orb.providers.*``) can depend on them without importing upward into the
application layer.

- :class:`ProviderHandler` — the abstract interface every provider handler
  satisfies (analogous to ``CommandHandler`` / ``QueryHandler``).
- :class:`BaseProviderHandler` — the shared template-method implementation with
  consistent logging, error handling, and per-request metrics.

Both depend only on domain ports (``LoggingPort``, ``ErrorHandlingPort``), so
the domain layer remains clean.
"""
...
__all__ = ["BaseProviderHandler", "ProviderHandler"]
