...
class DependencyResolver:
    ...
    def resolve(self, cls: Type[T], parent_type: Optional[Type] = None, 
    ...
    def _create_from_registration(self, registration: DependencyRegistration, 
                                dependency_chain: Set[Type]) -> Any:
        ...
        if registration.factory is not None:
            try:
                ...
                raise FactoryError(registration.dependency_type, f"Factory failed for {registration.dependency_type.__name__}: {str(e)}")
    ...
    def _resolve_constructor_parameters(self, cls: Type, dependency_chain: Set[Type]) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(e, (DependencyResolutionError, CircularDependencyError, UntypedParameterError)):
                raise
            else:
                raise DependencyResolutionError(cls, f"Failed to resolve constructor parameters for {cls.__name__}: {str(e)}")
    ...
    def _resolve_string_annotation(self, annotation: str, context_class: Type) -> Type:
        ...
        try:
            ...
            try:
                ...
                for module_name in ['typing', 'src.domain', 'src.application', 'src.infrastructure']:
                    try:
            ...
            raise DependencyResolutionError(context_class, f"Failed to resolve string annotation '{annotation}': {str(e)}")
    ...
    def _is_primitive_type(self, param_type: Type) -> bool:
        ...
        primitive_types = {
            ...
            type(None), list, dict, tuple, set, frozenset
        }
