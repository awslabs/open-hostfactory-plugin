...
from src.domain.base.exceptions import (
    DomainException, EntityNotFoundError, ValidationError
)
...
class TemplateConfigurationManager:
    ...
    def _batch_resolve_amis(self, template_dicts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(f"Batch resolved {len(resolved_amis)} unique SSM parameters for {len(template_dicts)} templates")
    ...
    async def validate_template(self, template: TemplateDTO, provider_instance: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Template validation completed for {template.template_id}: "
    ...
    async def _validate_with_provider_capabilities(self, template: TemplateDTO, 
