...
class TestPerformanceBenchmarks:
    ...
    def test_request_creation_performance(self):
        ...
        print(f"Request creation - Avg: {avg_time:.6f}s, Max: {max_time:.6f}s, Min: {min_time:.6f}s")
...
class TestMemoryPerformance:
    ...
    def test_memory_usage_during_request_creation(self):
        ...
        for i in range(iterations):
            ...
            if i % 100 == 0:
                ...
                assert memory_per_request < 10000, f"Memory per request too high: {memory_per_request} bytes"
        
        final_memory = process.memory_info().rss
        total_increase = final_memory - initial_memory
        memory_per_request = total_increase / iterations
        
        print(f"Memory usage - Total: {total_increase / 1024 / 1024:.2f} MB, Per request: {memory_per_request:.0f} bytes")
    ...
    def test_memory_leak_detection(self):
        ...
        if len(memory_samples) > 2:
            ...
            assert memory_per_iteration < 1024 * 1024, f"Potential memory leak: {memory_per_iteration / 1024:.0f} KB per iteration"
    ...
    def test_garbage_collection_impact(self):
        ...
        print(f"GC impact - Without GC: {time_without_gc:.4f}s, With GC: {time_with_gc:.4f}s, Overhead: {gc_overhead:.2%}")
...
class TestConcurrentPerformance:
    ...
    def test_concurrent_request_creation_performance(self):
        ...
        # Performance assertions
        assert avg_time_per_request < 0.01, f"Concurrent creation too slow: {avg_time_per_request:.6f}s"
    ...
    def test_thread_safety_performance_impact(self):
        ...
        print(f"Thread safety impact - Single: {single_thread_avg:.6f}s, Multi: {multi_thread_avg:.6f}s, Overhead: {overhead:.2%}")
...
class TestScalabilityLimits:
    ...
    def test_maximum_concurrent_requests(self):
        ...
        print(f"Concurrent requests - {successful_threads}/{max_threads} successful in {total_time:.2f}s")
    ...
    def test_large_dataset_handling(self):
        ...
        for size in large_dataset_sizes:
            ...
            assert time_per_item < 0.01, f"Time per item too high for {size} items: {time_per_item:.6f}s"
        ...
        assert scalability_factor < 5.0, f"Poor scalability: {scalability_factor:.2f}x slower for 20x data"
    ...
    def test_memory_scalability_limits(self):
        ...
        for size in dataset_sizes:
            ...
            print(f"Memory for {size} items: {memory_increase / 1024 / 1024:.2f} MB, {memory_per_item:.0f} bytes per item")
