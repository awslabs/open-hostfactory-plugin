        ...
        try:
            ...
            if storage_type == 'dynamodb':
                ...
                machine_repo = JSONMachineRepository(
                    file_path=os.path.join(data_dir, 'machines.json'),
                    create_dirs=True
                )
                
                request_repo = JSONRequestRepository(
                    file_path=os.path.join(data_dir, 'requests.json'),
                    create_dirs=True
                )
            ...
            try:
                ...
                if items:
                    ...
                    with open(backup_file, 'w') as f:
                        json.dump([item.to_dict() if hasattr(item, 'to_dict') else item for item in items], f, indent=2)
    ...
    def _migrate_collection(self, 
        ...
        try:
            ...
            for i in range(0, len(items), batch_size):
                ...
                self.logger.debug(f"Migrated batch {stats['batches']} ({i+1}-{min(i+batch_size, len(items))}) of {collection_name}")
