...
@router.get("/{request_id}/status", summary="Get Request Status", description="Get status of a specific request")
@handle_rest_exceptions(endpoint="/api/v1/requests/{request_id}/status", method="GET")
async def get_request_status(
    request_id: str,
    handler = Depends(get_request_status_handler())
) -> JSONResponse:
    """
...
async def list_requests(
    ...
    result = await handler.handle(
        status=status,
        limit=limit,
        context={"endpoint": "/requests", "method": "GET"}
    )
    
    return JSONResponse(content=result)


@router.get("/{request_id}", summary="Get Request Details", description="Get detailed information about a request")
