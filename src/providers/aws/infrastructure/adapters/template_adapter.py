...
class AWSTemplateAdapter(TemplateAdapterPort):
    ...
    _AWS_SUPPORTED_FIELDS = [
        'image_id', 'vm_type', 'vm_types', 'subnet_ids', 'security_group_ids',
        'key_name', 'iam_instance_profile', 'user_data', 'block_device_mappings',
        'placement', 'monitoring', 'ebs_optimized', 'instance_initiated_shutdown_behavior',
        'disable_api_termination', 'kernel_id', 'ramdisk_id', 'additional_info',
        'client_token', 'network_interfaces', 'dry_run', 'private_ip_address',
        'secondary_private_ip_address_count', 'associate_public_ip_address',
        'delete_on_termination', 'description', 'device_index', 'groups',
        'ipv6_address_count', 'ipv6_addresses', 'network_interface_id',
        'private_ip_addresses', 'subnet_id', 'fleet_type', 'spot_price',
        'target_capacity', 'allocation_strategy', 'diversified_allocation',
        ...
        if ami_id_or_alias.startswith('/'):
            try:
                ...
                if self._is_valid_ami_format(resolved_ami):
                    ...
                    raise AWSValidationError(f"SSM parameter {ami_id_or_alias} does not contain a valid AMI ID: {resolved_ami}")
    ...
    def _determine_provider_api(self, template: Template) -> str:
        ...
        if (getattr(template, 'spot_price', None) or 
            getattr(template, 'fleet_type', None) == 'request'):
            return 'SpotFleet'
...
def create_aws_template_adapter(aws_client: AWSClient, logger: LoggingPort, config: ConfigurationPort) -> AWSTemplateAdapter:
    """
