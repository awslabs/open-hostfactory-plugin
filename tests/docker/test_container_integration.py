...
class TestContainerIntegration:
    ...
    def test_container_authentication_configuration(self, built_image):
        ...
        try:
            result = subprocess.run([
                ...
                "-e", "HF_AUTH_BEARER_SECRET_KEY=test-secret-key",
                built_image, "version"
            ], capture_output=True, text=True, timeout=30)
    ...
    def test_container_server_startup(self, built_image):
        ...
        try:
            # Start container
            result = subprocess.run([
                "docker", "run", "-d", "--name", "ohfp-integration-test",
                "-p", "8003:8000",
                "-e", "HF_SERVER_ENABLED=true",
                "-e", "HF_AUTH_ENABLED=false",
    ...
    def test_container_bash_access(self, built_image):
        ...
        try:
            result = subprocess.run([
                "docker", "run", "--rm", built_image, "bash", "-c", "echo 'Container bash access works'"
            ], capture_output=True, text=True, timeout=30)
    ...
    def test_container_file_permissions(self, built_image):
        ...
        try:
            ...
            result = subprocess.run([
                "docker", "run", "--rm", built_image, "bash", "-c", 
                "ls -la /app/ | head -5"
            ], capture_output=True, text=True, timeout=30)
