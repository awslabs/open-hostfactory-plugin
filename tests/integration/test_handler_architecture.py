        ...
        for handler_class in handlers:
            ...
            for i, expected_param in enumerate(expected_params):
                if i >= len(params) or params[i] != expected_param:
                    print(f"   FAIL: {handler_name}: Missing or misplaced required parameter '{expected_param}'")
        ...
        for handler_class in handlers:
            ...
            for method_name, expected_params in expected_methods.items():
                ...
                if method_name == 'acquire_hosts':
                    param_annotations = [param.annotation for param in sig.parameters.values()][1:]  # Skip 'self'
                    if len(param_annotations) >= 2:
                        template_annotation = param_annotations[1]
                        if template_annotation != AWSTemplate and str(template_annotation) != 'AWSTemplate':
                            print(f"   FAIL: {handler_name}.{method_name}: Should use AWSTemplate, not {template_annotation}")
                            return False
...
def test_base_class_inheritance():
    ...
    try:
        ...
        for handler_class, handler_name in handlers:
            ...
            base_methods = ['_retry_with_backoff', '_convert_client_error', '_validate_prerequisites', 'get_metrics']
        ...
        for handler_class, handler_name in handlers:
            ...
            try:
                ...
                if not hasattr(handler, 'launch_template_manager'):
                    print(f"   FAIL: {handler_name}: launch_template_manager not stored as attribute")
                    return False
                ...
                print(f"   FAIL: {handler_name}: Failed to instantiate with launch template manager: {str(e)}")
                return False
    ...
    try:
        ...
        for method_name in error_methods:
            if not hasattr(handler, method_name):
                print(f"   FAIL: SpotFleetHandler: Missing inherited error handling method '{method_name}'")
                return False
    ...
    try:
        ...
        for handler_file in handler_files:
            ...
            if 'from src.domain.template.aggregate import Template' in content:
                print(f"   FAIL: {handler_file}: Still importing generic Template instead of AWSTemplate")
                return False
            ...
            if 'from src.providers.aws.infrastructure.handlers.base_handler import AWSHandler' not in content:
