...
class AWSAMIResolver(ImageResolver):
    ...
    def supports_reference_format(self, image_reference: str) -> bool:
        ...
        return (
            image_reference.startswith('ami-') or
            image_reference.startswith('/aws/service/') or
            self._is_custom_alias(image_reference)
        )
    ...
    def _is_custom_alias(self, reference: str) -> bool:
        ...
        return (
            len(reference) < 50 and
            not reference.startswith(('ami-', '/aws/', 'arn:')) and
            reference.replace('-', '').replace('_', '').isalnum()
        )
