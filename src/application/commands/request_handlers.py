...
from src.application.services.provider_capability_service import ProviderCapabilityService, ValidationLevel
...
class CreateMachineRequestHandler(BaseCommandHandler[CreateRequestCommand, str]):
    ...
    async def execute_command(self, command: CreateRequestCommand) -> str:
        ...
        try:
            ...
            self.logger.info(f"Selected provider: {selection_result.provider_instance} ({selection_result.selection_reason})")
            ...
            if is_dry_run:
                ...
                try:
                    ...
                    if provisioning_result.get('success'):
                        ...
                        request.metadata['handler_used'] = provisioning_result.get('provider_data', {}).get('handler_used', 'RunInstancesHandler')
                        ...
                        if isinstance(resource_ids, list):
                            for resource_id in resource_ids:
                                ...
                                if isinstance(resource_id, str):
                                    ...
                                    self.logger.error(f"Expected string resource_id, got: {type(resource_id)} - {resource_id}")
                        else:
                            self.logger.error(f"Expected list for resource_ids, got: {type(resource_ids)} - {resource_ids}")
                        ...
                        if len(instance_data_list) == command.requested_count:
                            from src.domain.request.value_objects import RequestStatus
                            request = request.update_status(RequestStatus.COMPLETED, "All instances provisioned successfully")
                        elif len(instance_data_list) > 0:
                            from src.domain.request.value_objects import RequestStatus
                            request = request.update_status(RequestStatus.PARTIAL, f"Partially fulfilled: {len(instance_data_list)}/{command.requested_count} instances")
                        else:
                            from src.domain.request.value_objects import RequestStatus
                            request = request.update_status(RequestStatus.IN_PROGRESS, "Resources created, instances pending")
                    else:
