...
class TestOCPComplianceIntegration:
    ...
    def test_command_handler_integration(self):
        ...
        # Execute command
        with patch('src.application.commands.provider_handlers.get_provider_registry', return_value=registry):
            result = handler.handle(command)
        
        # Verify strategy was registered with context
        mock_provider_context.register_strategy.assert_called_once_with(mock_strategy, "test-strategy")
        ...
        for file_path in files_to_check:
            if os.path.exists(file_path):
                ...
                for pattern in hard_coded_patterns:
                    assert pattern not in content, f"Found hard-coded conditional '{pattern}' in {file_path}"
