...
class HostFactorySchedulerStrategy(SchedulerPort):
    ...
    def __init__(self, config_manager: ConfigurationManager, logger: LoggingPort, template_defaults_service=None):
    ...
    def _map_template_fields(self, template: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.template_defaults_service:
            ...
            mapped['provider_api'] = template.get('providerApi', 
                                                template.get('provider_api', 'EC2Fleet'))
        ...
        self._logger.debug(f"Mapped template fields: {len(field_mappings)} HostFactory mappings applied for {provider_type} provider")
    ...
    def convert_cli_args_to_hostfactory_input(self, operation: str, args: Any) -> Dict[str, Any]:
        ...
        if operation == "requestMachines":
            ...
            return {
                "requests": [{
                    "requestId": getattr(args, 'request_id', '')
                }]
            }
    ...
    def _convert_template_to_hostfactory(self, template: Template) -> Dict[str, Any]:
        ...
        optional_fields = [
            'imageId', 'subnetId', 'vmType', 'vmTypes', 'keyName', 'securityGroupIds',
            'priceType', 'instanceTags', 'instanceProfile', 'userDataScript',
            'rootDeviceVolumeSize', 'volumeType', 'fleetRole', 'maxSpotPrice',
            'allocationStrategy', 'spotFleetRequestExpiry'
        ]
