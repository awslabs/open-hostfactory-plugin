...
class Provider1Strategy(ProviderStrategy):
    ...
    def execute_operation(self, operation: ProviderOperation) -> ProviderResult:
        ...
        try:
            ...
            return ProviderResult.error_result(
                f"Provider1 operation failed: {str(e)}",
                "PROVIDER1_ERROR"
            )
    ...
    def _get_instance_status(self, operation: ProviderOperation) -> ProviderResult:
        ...
        status_map = {
            instance_id: "running" for instance_id in instance_ids
        }
        
        return ProviderResult.success_result({
            "instance_status": status_map,
            "provider": "provider1"
        })
        ...
        try:
            if not self._client or not self._client.get("connected"):
                return ProviderHealthStatus.unhealthy(
                    "Provider1 client not connected"
                )
            ...
            return ProviderHealthStatus.healthy(
                f"Provider1 healthy - Region: {self._config.region}",
                response_time_ms
            )
...
def example_runtime_switching():
    ...
    for strategy_type in context.available_strategies:
        ...
        print(f"  {strategy_type}: {metrics.total_operations} ops, {metrics.success_rate:.1f}% success")
...
def example_load_balancing():
    ...
    load_balancer = LoadBalancingProviderStrategy(
        strategies=[aws_strategy, provider1_strategy],
        weights=weights,
        config=lb_config
    )
...
def example_production_monitoring():
    ...
    for strategy_type in context.available_strategies:
        ...
        print(f"{strategy_type:<12} {health_icon:<8} {metrics.total_operations:<6} "
