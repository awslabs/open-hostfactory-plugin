...
from src.providers.base.strategy.provider_strategy import ProviderCapabilities, ProviderOperationType
...
class ProviderCapabilityService:
    ...
    def validate_template_requirements(self, 
        ...
        try:
            ...
            self._logger.info(f"Validation result for {template.template_id}: {'VALID' if result.is_valid else 'INVALID'}")
    ...
    def _validate_api_support(self, template: Template, capabilities: ProviderCapabilities, result: ValidationResult):
        ...
        if template.provider_api not in supported_apis:
            result.errors.append(f"Provider does not support API '{template.provider_api}'. Supported APIs: {supported_apis}")
        else:
            result.supported_features.append(f"API: {template.provider_api}")
    
    def _validate_pricing_model(self, template: Template, capabilities: ProviderCapabilities, result: ValidationResult):
        """Validate pricing model support (spot/on-demand)."""
        if not template.provider_api:
            return
    ...
    def _validate_fleet_type_support(self, template: Template, capabilities: ProviderCapabilities, result: ValidationResult):
        ...
        if supported_fleet_types and fleet_type not in supported_fleet_types:
            result.errors.append(f"API '{template.provider_api}' does not support fleet type '{fleet_type}'. Supported types: {supported_fleet_types}")
        elif supported_fleet_types:
            result.supported_features.append(f"Fleet type: {fleet_type}")
    
    def _validate_instance_limits(self, template: Template, capabilities: ProviderCapabilities, result: ValidationResult):
