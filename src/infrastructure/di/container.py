...
from src.infrastructure.di.components import ServiceRegistry, CQRSHandlerRegistry, DependencyResolver
...
class DIContainer(DIContainerPort, CQRSHandlerRegistrationPort, ContainerPort):
    ...
    def __init__(self):
        ...
        logger.info(f"DI Container initialized (lazy_loading={'enabled' if self._lazy_config.enabled else 'disabled'})")
    ...
    def register_type(self, interface_type: Type[T], implementation_type: Type[T], 
                     registration_type=None) -> None:
    ...
    def get_command_handler(self, command_type: Type) -> Any:
        ...
        if handler_type is None:
            raise DependencyResolutionError(command_type, f"No command handler registered for {command_type.__name__}")
